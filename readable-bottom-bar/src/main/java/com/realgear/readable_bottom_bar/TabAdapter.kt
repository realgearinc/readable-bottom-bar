...
internal class TabAdapter(
    ...
    var onTabSelected: ((lastIndex: Int, lastTab: ReadableBottomBar.Tab?, newIndex: Int, newTab: ReadableBottomBar.Tab, animated: Boolean) -> Unit)? =
        null
    var onTabReselected: ((newIndex: Int, newTab: ReadableBottomBar.Tab) -> Unit)? = null
    var onTabIntercepted: ((lastIndex: Int, lastTab: ReadableBottomBar.Tab?, newIndex: Int, newTab: ReadableBottomBar.Tab) -> Boolean)? =
        null

    val tabs = ArrayList<ReadableBottomBar.Tab>()
    var selectedTab: ReadableBottomBar.Tab? = null
        private set
            ...
            payloads.isEmpty() -> holder.bind(tabs[position])
                ...
                when (payload.type) {
                    ...
                    PAYLOAD_UPDATE_BADGE ->
                        holder.applyBadge(payload.value as ReadableBottomBar.Badge?)
                    PAYLOAD_SELECT ->
    ...
    fun addTab(tab: ReadableBottomBar.Tab, tabIndex: Int = -1) {
        val addedIndex: Int
        if (tabIndex == -1) {
    ...
    fun addTabs(values: Array<out ReadableBottomBar.Tab>, tabIndex: Int = -1) {
        addTabs(NoCopyArrayList(values), tabIndex)
    }

    fun addTabs(values: Collection<ReadableBottomBar.Tab>, tabIndex: Int = -1) {
        val startIndex: Int
        if(tabIndex == -1) {
    ...
    fun removeTab(tab: ReadableBottomBar.Tab) {
    ...
    fun selectTab(tab: ReadableBottomBar.Tab, animate: Boolean) {
        val index = tabs.indexOf(tab)
        if(index >= 0) {
    ...
    fun applyTabBadge(tab: ReadableBottomBar.Tab, badge: ReadableBottomBar.Badge?) {
        val index = tabs.indexOf(tab)
        if(index >= 0) {
    ...
    fun applyTabBadgeAt(tabIndex: Int, badge: ReadableBottomBar.Badge?) {
        notifyItemChanged(tabIndex, Payload(PAYLOAD_UPDATE_BADGE, badge))
    }

    fun notifyTabChanged(tab: ReadableBottomBar.Tab) {
        val index = tabs.indexOf(tab)
        if (index >= 0) {
    ...
    fun applyIconSize(tab: ReadableBottomBar.Tab, iconSize: Int) {
        val index = tabs.indexOf(tab)
        if(index >= 0) {
    ...
    private fun canSelectTab(
        lastIndex: Int,
        lastTab: ReadableBottomBar.Tab?,
        newIndex: Int,
        newTab: ReadableBottomBar.Tab
    ): Boolean {
    ...
    inner class TabHolder(private val view: TabView) : RecyclerView.ViewHolder(view) {
        ...
        fun applyBadge(badge: ReadableBottomBar.Badge?) {
            view.badge = badge
        }
        ...
        fun bind(tab: ReadableBottomBar.Tab) {
            if (tab == selectedTab) {
                select(false)
            } else {
                deselect(false)
