...
class ReadableBottomBar @JvmOverloads constructor(
    ...
    private fun initAttributes(
        ...
        val attr: TypedArray =
            context.obtainStyledAttributes(attributeSet, R.styleable.ReadableBottomBar, 0, 0)
        try {
            // Type
            selectedTabType = TabType.fromId(
                attr.getInt(
                    R.styleable.ReadableBottomBar_rbb_selectedTabType,
            ...
            rippleEnabled = attr.getBoolean(
                R.styleable.ReadableBottomBar_rbb_rippleEnabled,
                tabStyle.rippleEnabled
            )
            rippleColor = attr.getColor(
                R.styleable.ReadableBottomBar_rbb_rippleColor,
                tabStyle.rippleColor
            )

            // Colors
            tabColorSelected = attr.getColor(
                R.styleable.ReadableBottomBar_rbb_tabColorSelected,
                tabStyle.tabColorSelected
            )
            tabColorDisabled = attr.getColor(
                R.styleable.ReadableBottomBar_rbb_tabColorDisabled,
                tabStyle.tabColorDisabled
            )
            tabColor =
                attr.getColor(R.styleable.ReadableBottomBar_rbb_tabColor, tabStyle.tabColor)

            // Text
            textAppearance =
                attr.getResourceId(
                    R.styleable.ReadableBottomBar_rbb_textAppearance,
                    tabStyle.textAppearance
                )
            val textStyle =
                attr.getInt(
                    R.styleable.ReadableBottomBar_rbb_textStyle,
                    typeface.style
                )
            typeface = Typeface.create(typeface, textStyle)
            textSize =
                attr.getDimensionPixelSize(
                    R.styleable.ReadableBottomBar_rbb_textSize,
                    tabStyle.textSize
                )
            ...
            iconSize =
                attr.getDimension(
                    R.styleable.ReadableBottomBar_rbb_iconSize,
            ...
            indicatorHeight =
                attr.getDimension(
                    R.styleable.ReadableBottomBar_rbb_indicatorHeight,
                    indicatorStyle.indicatorHeight.toFloat()
                ).toInt()
            indicatorMargin =
                attr.getDimension(
                    R.styleable.ReadableBottomBar_rbb_indicatorMargin,
                    indicatorStyle.indicatorMargin.toFloat()
                ).toInt()
            indicatorColor =
                attr.getColor(
                    R.styleable.ReadableBottomBar_rbb_indicatorColor,
                    indicatorStyle.indicatorColor
                )
            indicatorAppearance = IndicatorAppearance.fromId(
                attr.getInt(
                    R.styleable.ReadableBottomBar_rbb_indicatorAppearance,
                    indicatorStyle.indicatorAppearance.id
                )
            ) ?: indicatorStyle.indicatorAppearance
            indicatorLocation = IndicatorLocation.fromId(
                attr.getInt(
                    R.styleable.ReadableBottomBar_rbb_indicatorLocation,
                    indicatorStyle.indicatorLocation.id
                )
            ) ?: indicatorStyle.indicatorLocation
            indicatorAnimation = IndicatorAnimation.fromId(
                attr.getInt(
                    R.styleable.ReadableBottomBar_rbb_indicatorAnimation,
                    indicatorStyle.indicatorAnimation.id
                )
            ) ?: indicatorStyle.indicatorAnimation
            indicatorRectStyle = IndicatorStyle.fromId(
                attr.getInt(
                    R.styleable.ReadableBottomBar_rbb_indicatorStyle,
                    indicatorStyle.indicatorRectStyle.id
                )
            ) ?: indicatorStyle.indicatorRectStyle

            // Badge
            badgeAnimation = BadgeAnimation.fromId(
                attr.getInt(
                    R.styleable.ReadableBottomBar_rbb_badgeAnimation,
                    tabStyle.badge.animation.id
                )
            ) ?: tabStyle.badge.animation
            badgeAnimationDuration = attr.getInt(
                R.styleable.ReadableBottomBar_rbb_badgeAnimationDuration,
                tabStyle.badge.animationDuration
            )
            badgeBackgroundColor =
                attr.getColor(
                    R.styleable.ReadableBottomBar_rbb_badgeBackgroundColor,
                    tabStyle.badge.backgroundColor
                )
            badgeTextColor =
                attr.getColor(
                    R.styleable.ReadableBottomBar_rbb_badgeTextColor,
                    tabStyle.badge.textColor
                )
            badgeTextSize =
                attr.getDimensionPixelSize(
                    R.styleable.ReadableBottomBar_rbb_badgeTextSize,
                    tabStyle.badge.textSize
                )

            // Initials tabs
            val tabsResId = attr.getResourceId(R.styleable.ReadableBottomBar_rbb_tabs, -1)
            val initialIndex = attr.getInt(R.styleable.ReadableBottomBar_rbb_selectedIndex, -1)
            val initialTabId =
                attr.getResourceId(R.styleable.ReadableBottomBar_rbb_selectedTabId, -1)
        ...
        if (viewPager != null) {
            ...
            viewPager.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
                ...
                override fun onPageScrolled(
                    ...
                    val isSettling = previousState == ViewPager2.SCROLL_STATE_SETTLING

                    tabIndicator.setIndicatorOffset(position + positionOffset, isSettling)

                    if (previousState == ViewPager2.SCROLL_STATE_IDLE) {
                        tabIndicator.setIndicatorOffset(0F, true)
                    }
                }
                ...
                override fun onPageSelected(position: Int) {
                    if (userScrollChange) {
                        ...
                        tabIndicator.setIndicatorOffset(0F, true)
                        tabIndicator.enableAnimation = false
                        selectTabAt(position, true)
                        tabIndicator.enableAnimation = true
                    }
                }
            })
        }
    ...
    var indicatorRectStyle
        get() = indicatorStyle.indicatorRectStyle
        set(value) {
            indicatorStyle.indicatorRectStyle = value
            applyIndicatorStyle()
        }
    ...
    enum class IndicatorStyle(val id: Int) {
        NORMAL(0),
        HALVED(1);

        companion object {
            fun fromId(id: Int) : IndicatorStyle? {
                for (f in values()) {
                    if (f.id == id) return f
                }
                throw IllegalArgumentException()
