...
import com.realgear.readable_bottom_bar.utils.dpPx
import com.realgear.readable_bottom_bar.utils.getResourceId
...
internal class TabView @JvmOverloads constructor(
    ...
    private var _badge: ReadableBottomBar.Badge? = null
    ...
    var badge: ReadableBottomBar.Badge?
        get() = _badge
        set(value) {
            _badge = value
            updateBadge()
    ...
    private fun updateTabType() {
        animatedView = when (style.selectedTabType) {
            ReadableBottomBar.TabType.TEXT -> iconLayout
            ReadableBottomBar.TabType.ICON -> textLayout
        }

        selectedAnimatedView = when (style.selectedTabType) {
            ReadableBottomBar.TabType.TEXT -> textLayout
            ReadableBottomBar.TabType.ICON -> iconLayout
        }
    ...
    private fun updateColors() {
        ...
        when(style.selectedTabType) {
            ReadableBottomBar.TabType.ICON -> {
                iconTint = tabColorSelected
                textColor = tabColor
            }
            ReadableBottomBar.TabType.TEXT -> {
                iconTint = tabColor
    ...
    fun select(animate: Boolean = true) {
        ...
        if (animate && style.tabAnimationSelected != ReadableBottomBar.TabAnimation.NONE) {
            selectedAnimatedView.startAnimation(selectedInAnimation)
        } else {
            selectedAnimatedView.visibility = View.VISIBLE
        }

        if (animate && style.tabAnimation != ReadableBottomBar.TabAnimation.NONE) {
    ...
    private fun updateAnimations() {
        if (style.tabAnimationSelected != ReadableBottomBar.TabAnimation.NONE) {
        ...
        if (style.tabAnimation != ReadableBottomBar.TabAnimation.NONE) {
    ...
    private fun getAnimation(
        ...
        when(animationType) {
            ReadableBottomBar.TabAnimation.SLIDE -> {
                if (selected) {
                    valueFrom = when {
            ...
            ReadableBottomBar.TabAnimation.FADE -> {
                valueFrom = when {
            ...
            ReadableBottomBar.TabAnimation.NONE -> { }
        }
